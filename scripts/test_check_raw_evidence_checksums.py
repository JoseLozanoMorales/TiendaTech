...
class RawEvidenceChecksumTests(unittest.TestCase):
    ...
    def test_covers_json_headers_txt_svg_but_not_csv_or_md(self):
        count = generate(self.root, self.manifest, self.targets, self.single_files)
        self.assertEqual(count, 5)  # json + headers + txt + svg + resumen.json
        self.assertEqual(verify(self.root, self.manifest, self.targets, self.single_files), 5)
