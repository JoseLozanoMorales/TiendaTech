...
Mantenibilidad & Cobertura agregada 25{,}60\,\% sobre el árbol completo de los seis servicios Java (11{,}72--41{,}14\,\% por módulo; ninguno alcanza el 70\,\%) y PMD máximo 9; cifra regenerada el 14 de septiembre de 2026 tras retirar el filtro de alcance previo, ver §\ref{sec:cobertura-complejidad}.\\
...
La tabla \ref{tab:cobertura} procede del informe \path{informe-umbral-70.json}, regenerado el 14 de septiembre de 2026 sobre el árbol completo de cada servicio Java, tras retirar el filtro de alcance que antes limitaba la medición al paquete \texttt{application/**}. Las cifras resultantes son más bajas que las reportadas antes de ese ajuste, pero honestas: ningún servicio Java alcanza todavía el umbral del 70\,\%. \texttt{armado-ia}, no modificado en este trabajo, conserva su alcance y cifra previos.
...
Usuarios & 411 & 999 & 41,14\\
Ventas & 161 & 493 & 32,66\\
armado-ia & 405 & 540 & 75,00\\\midrule
Agregado (seis servicios Java) & 1506 & 5884 & 25,60\\\bottomrule
\end{tabular}
\end{table}

El umbral del 70\,\% se verifica directamente desde los siete XML mediante \evidencia{docs/evidencias/cobertura/verificar_umbral.py}{\texttt{verificar\_umbral.py}}. El resultado versionado \path{informe-umbral-70.json}, regenerado el 14 de septiembre de 2026 sobre una corrida limpia tras retirar el filtro de alcance que limitaba la medición al paquete \texttt{application/**} en los seis servicios Java, muestra que ninguno de esos seis módulos alcanza todavía el 70\,\%: entre 11,72\,\% (\texttt{ordenes-proveedores-service}) y 41,14\,\% (\texttt{usuarios}), con un agregado ponderado de 25,60\,\% sobre 5884 líneas instrumentables. El servicio \texttt{armado-ia}, no modificado en este trabajo, se mantiene en 75,00\,\%. La comprobación no depende de una publicación externa en Codecov y ahora mide el árbol completo de cada servicio Java, no solo su capa de aplicación. El umbral permanece declarado en el goal \texttt{check} de JaCoCo de cada \texttt{pom.xml}, con \texttt{haltOnFailure=false} para no bloquear la integración continua mientras se amplía la suite de pruebas hacia dominio, infraestructura y presentación.
