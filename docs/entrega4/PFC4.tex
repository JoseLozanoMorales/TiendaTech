...
% Punto 26: biblatex no trae traduccion al espanol de "artno" (numero de
% articulo, usado por el campo eid de daraghmi2022saga) en la imagen de
% TeX Live que usa CI -- sin esto imprime el nombre interno de la cadena tal
% cual ("artno 6242") en vez de un texto legible. Ver PFC4.log de esa
% corrida: "Bibliography string 'artno' untranslated".
% \NewBibliographyString es idempotente (blx@newstring no hace nada si
% la clave ya existe) -- con esto el fix funciona tanto si "artno" ya
% esta declarada por el estilo ieee (como en la imagen de CI, que sí la usa
% para el campo eid) como si no lo esta en otra instalacion de TeX Live,
% en vez de fallar con "Package keyval Error: artno undefined".
...
\newcommand{\evidencia}[2]{\href{https://github.com/JoseLozanoMorales/TiendaTech/blob/c5dca588ea9662cab56732c97fe644b9a929854b/#1}{#2}}
...
\textbf{Objetivo:} consolidar la evolución de TiendaTech y evaluar qué propiedades de su arquitectura distribuida quedan respaldadas por evidencia. \textbf{Método:} revisión del repositorio, mediciones de CockroachDB y Spark, y una campaña correctiva de 120 corridas mediante API Gateway, microservicios y CockroachDB, con dos estrategias, cuatro concurrencias y tres condiciones de fallo. \textbf{Resultados:} la matriz real contiene 208003 solicitudes y 49786 intentos de checkout, de los cuales 30275 fueron confirmados. Saga confirmó 63,4\,\% y 2PC 58,1\,\% en el agregado descriptivo; ninguna de las doce comparaciones de p95 resultó significativa con Bonferroni. Un oráculo retrospectivo sobre CockroachDB encontró 13210 órdenes con movimiento de inventario ausente o distinto entre 43168 órdenes persistidas; no halló importes discordantes, descuentos duplicados ni stock negativo, mientras la compensación quedó no verificable. La evaluación ISO observó 3588/3588 sondeos exitosos durante una hora y un p95 de 610\,ms con 50 usuarios, superior al objetivo de 500\,ms. \textbf{Contribución:} una memoria trazable que separa implementación, medición y límites de inferencia. No se acredita una comparación reglas--RAG.

\textbf{Palabras clave:} sistemas distribuidos, coordinación, CockroachDB, Spark, calidad, reproducibilidad.
\clearpage
\section*{Abstract}
\addcontentsline{toc}{section}{Abstract}
\textbf{Objective:} to consolidate TiendaTech's evolution and determine which distributed-system properties are supported by evidence. \textbf{Method:} repository review, CockroachDB and Spark measurements, and a corrective campaign of 120 runs through the API Gateway, microservices and CockroachDB, covering two strategies, four concurrency levels and three fault conditions. \textbf{Results:} the real matrix contains 208,003 requests and 49,786 checkout attempts, of which 30,275 were confirmed. Saga confirmed 63.4\,\% and 2PC 58.1\,\% in the descriptive aggregate; none of the twelve p95 comparisons was significant after Bonferroni correction. A retrospective CockroachDB oracle found 13,210 orders with missing or mismatched inventory movements among 43,168 persisted orders; it found no amount mismatches, duplicate discounts or negative stock, while compensation remained unverifiable. The ISO assessment observed 3,588 successful probes out of 3,588 during one hour and a p95 of 610 ms with 50 users, above the 500 ms target. \textbf{Contribution:} a traceable report separating implementation, measurement and inference limits. A rules-versus-RAG comparison is not established.
...
El corte histórico de la macro \texttt{\textbackslash evidencia} integra las evidencias publicadas hasta el commit \texttt{c5dca58}, cuyo hash completo fijan sus enlaces. Los reportes web posteriores se citan mediante enlaces individuales a commits publicados con hash completo, según el registro de verificación del 12 de septiembre. Incluye la campaña correctiva del 5 de septiembre, la evaluación ISO, las pruebas de CI, el panel Grafana y las trazas. Cada resultado conserva su procedencia y unidad experimental; el cierre de esta memoria no equivale a declarar aprobados los pasos técnicos que permanecen como deuda.
...
La tabla \ref{tab:cap} (``Consistencia por agregado'') formula una política CAP distinta para inventario, pedido/pago, usuarios, catálogo y analítica, cada una con su límite de evidencia declarado. \textbf{Actualización del 7 de septiembre:} la campaña correctiva de checkout produjo 30275 confirmaciones en 120 corridas contra el sistema desplegado, después de aprobar pilotos basales y una rampa de carga, y un oráculo retrospectivo auditó esas mismas corridas directamente contra CockroachDB real (no contra el CSV agregado). El oráculo evalúa parcialmente invariantes por agregado, con límite declarado, pero no valida CAP porque no se indujo una partición de red controlada: cero discordancias de importe entre orden y factura, cero descuentos duplicados y cero stock negativo, frente a 30,6\,\% de órdenes con inconsistencia de inventario. Ninguna de las dos estrategias (2PC, Saga) garantizó por sí sola esa consistencia de inventario bajo esta concurrencia, y la comparación entre ambas no fue estadísticamente significativa tras Bonferroni. La cuarta invariante (compensación de intentos cancelados) sigue sin poder verificarse, no por falta de intento sino porque su registro vivía solo en memoria y se perdió entre reinicios; se declara \texttt{no\_verificado} en vez de asumirse cumplida.\\
...
La tabla \ref{tab:cap} formula la política requerida por agregado y ahora la sostiene con datos medidos, no solo argumentados: el oráculo retrospectivo del Paso 8 auditó las 120 corridas de la campaña correctiva contra CockroachDB real (\path{experiments/paso8/resultados-reales/correctiva-20260905-final-v2/analisis/oracle_resumen_crdb.json}). CAP se interpreta ante particiones de red: la réplica por quórum puede rechazar o demorar escrituras cuando no dispone de mayoría. No se promete disponibilidad total junto con consistencia fuerte durante cualquier partición \cite{gilbert2002,brewer2012cap,abadi2012consistency}.

\begin{table}[H]\centering\small
\caption{Consistencia, réplica y límites por agregado, sostenidos con el oráculo del Paso 8.}\label{tab:cap}
\begin{tabularx}{\textwidth}{lXX}
\toprule Agregado & Política & Evidencia medida\\\midrule
...
Pedido y pago & Confirmar solo con condiciones satisfechas; compensar fallo. & Cero discordancias de importe o de líneas entre orden y factura en las 36\,878 órdenes con factura persistida. La comparación 2PC/Saga no mostró diferencia significativa tras la corrección de Bonferroni ($\hat{A}_{12}$ entre 0,6 y 0,92 sin ajustar); la compensación de intentos cancelados queda \texttt{no\_verificado} por pérdida del registro en memoria entre reinicios.\\
...
Analítica & Instantánea o extracción identificable. & Resultados derivados pueden quedar desfasados respecto de ventas; no evaluado en esta campaña.\\\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]\centering\small
\caption{Oráculo retrospectivo sobre la instantánea de CockroachDB.}\label{tab:oraculo-crdb}
\begin{tabular}{lr}
\toprule Comprobación persistente & Resultado\\\midrule
...
Corridas con violación observable & 104 de 120\\\bottomrule
\end{tabular}
\end{table}

El oráculo usó una instantánea de solo lectura y no alteró el CSV crudo. La tabla \ref{tab:oraculo-crdb} separa los hallazgos persistentes de los estados no conservados: 16 corridas quedan \texttt{no\_verificado}, nunca aprobadas por ausencia de datos.
...
\textbf{Reconstrucción retrospectiva.} La asignación de órdenes usa ventanas temporales y una instantánea MVCC posterior a la campaña. Permite auditar relaciones persistentes, pero 13321 órdenes sintéticas quedaron fuera de las ventanas y no reconstruye causalidad ni estados efímeros. Los 13210 desajustes de inventario son violaciones observables; los ceros de otras consultas no equivalen a riesgo nulo. Una réplica debe persistir identificadores y estados parciales y añadir controles positivos para cada invariante.
...
\textbf{Oráculo incompleto.} No existe ledger de cobros y el buffer de 200 observaciones se perdió entre reinicios; la compensación cancelada queda \texttt{no\_verificado}. Mitigación: persistir el ciclo completo por identificador y probar que mutaciones deliberadas de cada invariante son detectadas.
...
Mantenibilidad & Cobertura agregada 32{,}28\,\% sobre el árbol completo de los ocho servicios medidos (seis Java, el Gateway y \texttt{armado-ia}; 11{,}72--78{,}26\,\% por módulo; solo el Gateway alcanza el 70\,\%) y PMD máximo 9; cifra regenerada el 18 de septiembre de 2026 en una corrida de CI que también agrega JaCoCo al Gateway y amplía \texttt{armado-ia} a su paquete \texttt{app} completo, ver §\ref{sec:cobertura-complejidad}.\\
...
\subsection{Cobertura y complejidad}\label{sec:cobertura-complejidad}
La tabla \ref{tab:cobertura} procede del informe \path{informe-umbral-70.json}, regenerado el 18 de septiembre de 2026 sobre el árbol completo de cada servicio Java, el paquete \texttt{app} completo de \texttt{armado-ia} y el Gateway (\texttt{Apps/web/frontend}), que hasta entonces no tenía JaCoCo configurado. Las cifras resultantes son más bajas que las reportadas antes de ese ajuste, pero honestas: solo el Gateway alcanza el umbral del 70\,\%.
...
Inventario & 311 & 1396 & 22,28\\
Órdenes a proveedores & 64 & 546 & 11,72\\
Pedidos & 443 & 1938 & 22,86\\
Productos & 116 & 512 & 22,66\\
Usuarios & 442 & 1037 & 42,62\\
Ventas & 161 & 493 & 32,66\\
Gateway & 162 & 207 & 78,26\\
armado-ia & 580 & 931 & 62,30\\\midrule
Agregado (ocho servicios) & 2279 & 7060 & 32,28\\\bottomrule
\end{tabular}
\end{table}

El umbral del 70\,\% se verifica directamente desde los ocho XML mediante \evidencia{docs/evidencias/cobertura/verificar_umbral.py}{\texttt{verificar\_umbral.py}}. El resultado versionado \path{informe-umbral-70.json}, regenerado el 18 de septiembre de 2026 sobre una corrida limpia de los ocho servicios, muestra que solo el Gateway alcanza el 70\,\% (78,26\,\%): los otros siete quedan entre 11,72\,\% (\texttt{ordenes-proveedores-service}) y 62,30\,\% (\texttt{armado-ia}), con un agregado ponderado de 32,28\,\% sobre 7060 líneas instrumentables. La comprobación no depende de una publicación externa en Codecov y mide el árbol completo de cada servicio Java, el paquete \texttt{app} completo de \texttt{armado-ia} (antes restringido a \texttt{app.domain} y \texttt{app.explicacion}) y, desde esta corrida, el Gateway. El umbral permanece declarado en el goal \texttt{check} de JaCoCo de cada \texttt{pom.xml}, con \texttt{haltOnFailure=false} para no bloquear la integración continua mientras se amplía la suite de pruebas hacia dominio, infraestructura y presentación.

Las cifras de \texttt{Usuarios} y \texttt{Productos} se corrigieron el 18 de septiembre de 2026: una primera medición local había reportado 511/1037 (49,28\,\%) y 138/512 (26,95\,\%) porque la corrida en IntelliJ tenía activo el perfil Maven \texttt{pact}, que agrega \texttt{src/test/pact-java/} (pensado para correr solo con \texttt{-Ppact} en los jobs de verificación de contrato de CI) al \texttt{mvn test} plano. La primera corrida en vivo del job \texttt{coverage-java} en CI, sin ese perfil, ya midió los valores correctos, confirmados localmente con \texttt{mvn clean test} y el perfil \texttt{pact} desactivado.

El resumen regenerado en \path{docs/experimentos/resultados/iso25010/complejidad/summary.csv}, que acompaña al código refactorizado, aplica el mismo criterio al servidor Java y al cliente TypeScript/React: máximo de complejidad ciclomática por método o función estrictamente inferior a diez. PMD 7.17.0 mide Java y la regla \texttt{complexity} de ESLint mide la web en su variante clásica; cada herramienta cuenta las decisiones propias de su lenguaje. La ejecución local posterior al refactor volvió a medir los siete módulos Java completos, sin excluir los métodos modificados. Los siete servicios Java y el cliente web cumplen el umbral. No se extiende esta medición al servicio Python. El \href{https://github.com/JoseLozanoMorales/TiendaTech/blob/f6de7608d79b746d11a4c14440c89086d765b279/docs/experimentos/resultados/iso25010/complejidad/summary.csv}{resumen inicial anterior al refactor} se conserva como referencia histórica y no contiene los valores de la tabla actual.

\begin{center}
\begin{tabular}{lrrl}
\hline
Módulo & Máximo & Objetivo & Estado\\
\hline
Gateway & 7 & $<10$ & Cumple\\
Inventario & 8 & $<10$ & Cumple\\
Órdenes a proveedores & 8 & $<10$ & Cumple\\
Pedidos & 9 & $<10$ & Cumple\\
Productos & 8 & $<10$ & Cumple\\
Usuarios & 9 & $<10$ & Cumple\\
Ventas & 8 & $<10$ & Cumple\\
Cliente web & 9 & $<10$ & Cumple\\
\hline
\end{tabular}
\end{center}

La medición web analiza el código de producción de \texttt{src} (TypeScript y TSX), incluyendo componentes y callbacks; excluye pruebas y herramientas fuera de ese directorio. Se fuerza el umbral de reporte a cero para registrar todas las funciones, incluidas las de complejidad uno, sin permitir supresiones inline. La ejecución local del 13 de septiembre de 2026, después del refactor, registró 434 funciones en 19 archivos de producción y un máximo de 9. Los valores, ubicaciones y versión de ESLint se conservan en \path{docs/experimentos/resultados/iso25010/complejidad/webapp-complexity.json}, junto al resumen regenerado. La \href{https://github.com/JoseLozanoMorales/TiendaTech/blob/f6de7608d79b746d11a4c14440c89086d765b279/docs/experimentos/resultados/iso25010/complejidad/webapp-complexity.json}{medición web inicial} registraba 404 funciones y un máximo de 26 en \texttt{api}; el refactor separó responsabilidades HTTP y secciones de presentación, manteniendo el umbral y el alcance del análisis. El comando \texttt{npm run complexity:report} actualiza la fila web del resumen conjunto y devuelve un código de error si el máximo alcanza diez. El job \texttt{web-quality} descarga la medición PMD de la misma ejecución de CI, añade la medición web y conserva ambas en el artefacto. La medición local ahora cumple; su resultado no sustituye la confirmación de la siguiente ejecución remota de CI. Las pruebas web se ejecutan también cuando falla la complejidad, para conservar diagnósticos independientes.

El análisis general de TypeScript y React Hooks se mantiene separado mediante \texttt{npm run lint:report}, con sus reportes \texttt{webapp-eslint.json} y \texttt{webapp-eslint-summary.csv}, y sigue fallando ante errores o advertencias. Sus recuentos de incidencias no se usan como sustituto de la complejidad ciclomática.


La cobertura del cliente se publica por separado en el \href{https://github.com/JoseLozanoMorales/TiendaTech/blob/ef374a09d203fc56cd21d08d7b7922ce9f42016c/docs/evidencias/cobertura/web/coverage-summary.json}{resumen de cobertura web}, regenerado el 14 de septiembre de 2026 tras retirar de \texttt{vitest.config.ts} la lista de doce vistas seleccionadas a mano: 62 pruebas y 71,96\,\% de líneas sobre el árbol completo de \texttt{src} (antes 19 pruebas y 53,39\,\% sobre solo tres vistas). Se conservan además sentencias (56,26\,\%), ramas (45,10\,\%) y funciones (44,47\,\%), con servicios HTTP simulados. La ampliación de alcance deja visibles dos huecos que la medición anterior no cubría: \texttt{App.tsx}, el punto de entrada de la aplicación, y \texttt{src/services/cart.ts}, la lógica de carrito, están en 0\,\% de cobertura por no haber sido medidos antes. Estos resultados no se agregan a la cobertura del backend ni se someten automáticamente al umbral del 70\,\% de los ocho XML anteriores.

El 71,96\,\% de líneas es la cifra menos representativa de las cuatro y no
debe leerse como el resumen de la cobertura web: Istanbul cuenta ``líneas''
por línea física del archivo, y varios componentes de \texttt{src/views/}
están escritos con muy pocas líneas físicas por mucho código (JSX y lógica
condensados), lo que infla ese porcentaje respecto a sentencias y ramas. El
propio \texttt{coverage-summary.json} lo muestra: \texttt{CheckoutView.tsx}
reporta 1/1 línea (100\,\%) pero solo 24/32 sentencias (75\,\%) y 9/21 ramas
(42,85\,\%); \texttt{AccountView.tsx} reporta 2/2 líneas (100\,\%) frente a
35/40 sentencias (87,5\,\%) y 28/44 ramas (63,63\,\%); \texttt{RecoveryView.tsx}
y \texttt{WorkerView.tsx} reportan 1/1 línea (100\,\%) con 15 y 11 sentencias
reales respectivamente. Sentencias (56,26\,\%) y ramas (45,10\,\%) miden sobre
unidades de código que no dependen del formato de línea del archivo, así que
son las cifras que mejor reflejan cuánto del comportamiento real de la SPA
está bajo prueba; el 71,96\,\% se conserva por ser la métrica que reporta
Vitest por defecto, no porque sea la más honesta de las cuatro.


\subsection{CI, carga y observabilidad}
Se verificó la demostración rojo/verde de GitHub Actions mediante dos ejecuciones concretas de \emph{CI-CD quality gate}: la \href{https://github.com/JoseLozanoMorales/TiendaTech/actions/runs/33554959074}{ejecución roja} concluyó en \texttt{failure} sobre el commit \texttt{f58869b}, mientras que la \href{https://github.com/JoseLozanoMorales/TiendaTech/actions/runs/33557738789}{ejecución verde} concluyó en \texttt{success} sobre \texttt{bd027f0}. Ambas fueron disparadas por \texttt{pull\_request}; los PR \#81, \#82 y \#83 conservan la secuencia de fallo intencional, reversión y corrección final. Son ejecuciones de CI, no solicitudes de incorporación de cambios. La publicación de imágenes en GitHub Container Registry (GHCR) pertenece a otro workflow, definido en \path{.github/workflows/publish-images.yml}, y se activa después de que el quality gate de \texttt{main} finaliza satisfactoriamente o mediante ejecución manual; la migración está registrada en el commit \texttt{a922bf4}.
...
El punto 17 de la guía de cierre observó que ninguna captura citada mostraba los cinco paneles del dashboard con series de datos a la vez, que el manuscrito seguía enlazando solo la captura antigua de \path{paso10-item5-grafana-carga.md} y que no mencionaba la inyección de fallos usada para producir el panel \texttt{5xx}. Se cerró con evidencia adicional en \href{https://github.com/JoseLozanoMorales/TiendaTech/blob/f3520a4/docs/evidencias/punto17-observabilidad-gateway.md}{\texttt{punto17-observabilidad-gateway.md}}: el Gateway (\texttt{Apps/web/frontend}) quedó instrumentado igual que los seis microservicios Java y scrapeado por Prometheus; el eje de los paneles \texttt{4xx}/\texttt{5xx} tenía la unidad de Grafana mal configurada (la consulta PromQL devuelve una fracción 0--1, no un porcentaje 0--100), lo que explica por qué esos paneles aparecían planos en capturas anteriores del proyecto pese a que el \texttt{429} real sí llegaba al Gateway; y, dado que el limitador de tasa nunca deja pasar tráfico suficiente para un \texttt{5xx} orgánico, se usó el mecanismo de inyección de fallos ya existente en el código (\texttt{ExperimentFaultInjector} en \texttt{ventas-service}, activado por el encabezado \texttt{X-Failure-Mode: omission} y una variable de entorno desactivada por defecto) para producir fallos reales de checkout. Una corrida de carga de 400 usuarios generó 33\,006 solicitudes con 3\,812 \texttt{429} reales, y cuatro de cinco checkouts disparados con el inyector de fallos activo devolvieron un \texttt{5xx} real; las dos capturas resultantes (\path{dashboard-gateway-trafico-latencia.png} y \path{dashboard-gateway-5xx-real.png}) muestran, entre ambas, los cinco paneles con series simultáneas y datos reales, incluido el panel \texttt{5xx} subiendo para \texttt{pedidos} y \texttt{ventas} —los servicios que participan directamente en el checkout con fallo inyectado—, mientras que \texttt{gateway} se mantiene cerca de 0\,\% en ese mismo panel porque su tráfico total, dominado por la carga pública, diluye la misma cantidad absoluta de fallos a una fracción mucho menor; no es ausencia de instrumentación. Se corrigió además un problema real detectado en los \texttt{.csv} crudos de esta corrida: Locust, en la máquina Windows donde se generaron, los escribía con doble retorno de carro (\texttt{\textbackslash r\textbackslash r\textbackslash n}) en vez de \texttt{\textbackslash r\textbackslash n}, lo que producía filas vacías intercaladas al leerlos con un lector CSV estándar; se normalizó la salida ya generada y se corrigió \texttt{run-load-test.ps1} para que ninguna corrida futura vuelva a quedar así. Por último, se agregó una prueba (\texttt{GatewayTrafficFilterTest.registraMetricasPrometheusConNombreYStatusReales}) que construye el filtro del Gateway con un \texttt{MeterRegistry} real y falla si se renombra la métrica o se fija el \texttt{status} a un valor constante en vez del código de respuesta real —la mutación exacta bajo la cual las pruebas anteriores permanecían en verde. Queda pendiente proteger o justificar formalmente la exposición sin autenticación de \texttt{/metrics} y \texttt{/health} en el Gateway, y todavía no existe un registro de datos crudos de la corrida de inyección de fallos citada (solo las capturas), aunque el script ya quedó preparado para generarlo en la próxima corrida.
...
Eval. E2 & Integración y validación automática de referencias; sin estimación acordada. & El CSV de seguridad y los tres enlaces web se verificaron por separado; ese control puntual no acredita todos los destinos del documento. & Integración: acordar el formato con la tarea 2, ejecutar el validador cuando esté disponible y regenerar el PDF final.\\
\bottomrule\end{longtable}}

Las observaciones de evaluación E4 (GHCR), E6 (APK y checksum) y E8 (carpetas huérfanas) cuentan con los commits registrados en la tabla \ref{tab:cambios-entregas} y no constituyen pendientes documentales de esta tabla. Estos códigos de evaluación no designan entregas. Eval. E2 tiene resuelta la cita de seguridad: el CSV del 11 de septiembre registra ocho comprobaciones HTTP 401 y coincide con las ocho rutas del test parametrizado, dentro de una suite de once pruebas. La evidencia fue publicada en \href{https://github.com/JoseLozanoMorales/TiendaTech/commit/b8d3e23988bf32e739f260e21cb8b999f362e783}{\texttt{b8d3e23}} y sustituye la referencia histórica al CSV ausente del 1 de septiembre. El reporte local de la suite indica once pruebas sin fallos, pero no está versionado y no certifica por sí solo la fecha ni la revisión ejecutada. La corrección de esta cita no cierra todos los enlaces de E2 ni acredita un validador automático: los tres destinos web se corrigen y comprueban por separado en esta revisión; la integración y la validación automática permanecen pendientes. El cierre de E6 acredita un paquete release firmado, su integridad, procedencia y publicación automática. José confirmó su instalación e inicio en dos dispositivos; esa confirmación no acredita una prueba integral de compra ni registra modelos o versiones de Android.
...
\textbf{Pregunta 1: invariantes.} La respuesta es parcial y revela fallos reales. El oráculo retrospectivo halló 13210 órdenes con movimiento de inventario ausente o distinto entre 43168 órdenes persistidas y 104 corridas con alguna violación observable. No halló discordancias de importe o líneas, descuentos duplicados ni stock negativo. No puede acreditar captura bancaria ni compensación completa porque esos estados no se persistieron. Por tanto, la implementación no conservó todas las invariantes y la evidencia restante se declara \texttt{no\_verificado}.
